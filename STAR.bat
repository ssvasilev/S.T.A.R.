...
if "!GITHUB_OK!"=="false" (
    echo ?? Не удалось получить данные о релизах GitHub
    echo.
) else (
    ...
    if not "!LATEST_LIVE_VERSION!"=="не найдена" (
        ...
        echo ?? LIVE-тег среди последних релизов не найден
    )
    if not "!LATEST_PTU_VERSION!"=="не найдена" (
        echo ? PTU по тегу:  !LATEST_PTU_VERSION! ^(тег: !LATEST_PTU_TAG!^)
    ) else (
        echo ?? PTU-тег среди последних релизов не найден
    )
    echo.
)
...
if /i not "!ARCHIVE_BUILD_TYPE!"=="!CURRENT_INSTALL!" (
    echo.
    echo ?? ВНИМАНИЕ: В архиве находится сборка типа !ARCHIVE_BUILD_TYPE!
...
if not "!ARCHIVE_VERSION_DIGITS!"=="!TARGET_VERSION!" (
    echo.
    echo ?? ВНИМАНИЕ: Версия в архиве не совпадает с ожидаемой по тегу.
...
if "!backup_result!"=="0" (
    ...
    echo ?? Не удалось создать резервную копию user.cfg. Настройка user.cfg пропущена.
)
...
if /i "%raw_status%"=="устарела" set "cell_text=?? устарела              "
...
if "!GITHUB_OK!"=="false" (
    echo ?? Не удалось получить данные об обновлениях с GitHub.
    echo.
    goto :eof
)

if /i "!INSTALL_LIVE_RESULT!"=="подходящих тегов больше нет" (
    echo ?? LIVE не была обновлена: подходящие теги закончились.
)
if /i "!INSTALL_PTU_RESULT!"=="подходящих тегов больше нет" (
    echo ?? PTU не была обновлена: подходящие теги закончились.
)
if /i "!INSTALL_LIVE_RESULT!"=="ошибка выбора тега" (
    echo ?? LIVE не была обновлена: не удалось определить тег.
)
if /i "!INSTALL_PTU_RESULT!"=="ошибка выбора тега" (
    echo ?? PTU не была обновлена: не удалось определить тег.
)
